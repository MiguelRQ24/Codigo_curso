...
def esAntisimetrica(matriz):
    for (vueltas, fila) in enumerate(matriz):
        for posicion in range(0, len(fila)):
            if len(matriz) != len(fila):                                                 
                return False
            elif posicion == vueltas:                                                     
                continue
            elif matriz[vueltas][posicion] != -matriz[posicion][vueltas]:                 
                return False
            else:
                continue
    return True
